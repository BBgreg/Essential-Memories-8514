-- Create the main memory storage table with exact schema requirements
CREATE TABLE IF NOT EXISTS dates_esm1234567 (
    ...
    category TEXT NOT NULL CHECK (category IN ('Birthday', 'Anniversary', 'Special Date', 'Holiday')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL
);

-- Create streak data table with exact schema requirements
...
-- Create practice sessions table for tracking user performance
...
-- Create RLS policies for dates_esm1234567 (CRITICAL for user isolation)
...
-- Create function to handle new user profile creation (CRITICAL for foreign key linkage)
...
-- Create trigger for automatic profile creation (ABSOLUTELY CRITICAL)
...
-- Create indexes for performance (supporting 40+ memories per user)
